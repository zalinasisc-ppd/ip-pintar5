...
EXPECTED_COLUMNS = {
    ...
    "GPMP PPC": ["GPMP PPC", "PPC"],
    "GPMP OTR2": ["GPMP OTR2", "OTR2"],
}
...
def compute_ipmp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = ["Mata Pelajaran", "Bil. Ambil", "GPMP PPC", "GPMP OTR2"]
    for c in needed:
        if c not in df.columns:
            raise ValueError(f"Kolum '{c}' tiada dalam data.")
    # Coerce numerics
    for c in ["Bil. Daftar", "Bil. Ambil", "GPMP PPC", "GPMP OTR2"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)

    # Calculations
    df["Beza GPMP (OTR2 - PPC)"] = df["GPMP OTR2"] - df["GPMP PPC"]
    total_ambil = df["Bil. Ambil"].sum()
    df["Pemberat"] = np.where(total_ambil > 0, df["Bil. Ambil"] / total_ambil, 0.0)
    df["IPMP"] = df["Beza GPMP (OTR2 - PPC)"] * df["Pemberat"]
    ...
    cols = [
        ...
        "GPMP PPC", "GPMP OTR2",
        "Beza GPMP (OTR2 - PPC)", "Pemberat", "IPMP", "Ranking"
    ]
...
st.sidebar.markdown("---")
st.sidebar.caption("Kolum diperlukan: Mata Pelajaran, Bil. Ambil, GPMP PPC, GPMP OTR2. (Bil. Daftar opsyenal).")
...
long_df = df_result.melt(id_vars=["Mata Pelajaran"], value_vars=["GPMP PPC", "GPMP OTR2"],
                         var_name="Jenis", value_name="GPMP")
fig2 = px.line(long_df, x="Mata Pelajaran", y="GPMP", color="Jenis",
               markers=True, title="GPMP OTR2 vs PPC Mengikut Subjek")
